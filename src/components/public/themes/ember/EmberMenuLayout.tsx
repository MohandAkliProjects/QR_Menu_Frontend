...
import EmberDishCard from "./EmberDishCard";
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          boxShadow:
            "0 0 0 2px var(--menu-accent), 0 0 0 5px var(--menu-bg), 0 4px 14px rgba(0,0,0,0.45)",
        }}
      >
        {hasImage ? (
          <img
      ...
      <span className="text-[11px] font-bold tracking-[0.14em] uppercase text-(--menu-primary) text-center leading-tight">
        {label}
      </span>
    </button>
  );
...
export default function EmberMenuLayout({
  menu,
  language,
  availableLanguages,
  onLanguageChange,
      ...
      <div className="w-full sm:max-w-2xl lg:max-w-5xl mx-auto px-4 pb-24">
        ...
        <div className="relative pt-6 pb-4 flex flex-col items-center text-center">
          <div
            ...
            style={{
              background:
                "radial-gradient(circle at 50% 20%, rgba(209,85,43,0.28), transparent 70%)",
            }}
          />

          {menu.restaurant.logoUrl ? (
            <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl overflow-hidden border border-(--menu-border) shadow-lg bg-(--menu-card) mb-3">
              <img
            ...
            <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl border border-(--menu-border) shadow-lg bg-(--menu-secondary) flex items-center justify-center mb-3">
              <span className="text-2xl sm:text-3xl font-bold text-(--menu-accent) menu-font-display">
          ...
          {menuTitle && (
            <p className="relative text-[10px] font-bold tracking-[0.2em] text-(--menu-accent) uppercase mb-1">
              {menuTitle}
            </p>
          )}

          <h1 className="relative text-2xl sm:text-3xl font-bold text-(--menu-primary) menu-font-display leading-tight">
            {menu.restaurant.name}
          </h1>

          {(menu.restaurant.address || menu.restaurant.ville) && (
            <p className="relative text-sm text-(--menu-muted) mt-1">
              {menu.restaurant.address || menu.restaurant.ville}
            </p>
          )}
      ...
      >
        <div className="relative shrink-0" style={{ height: 240 }}>
          ...
          <button
            type="button"
            onClick={onClose}
            className="absolute top-4 inset-e-4 w-9 h-9 rounded-full bg-black/45 backdrop-blur-sm flex items-center justify-center text-white"
            aria-label={t.close}
          >
            <X className="w-4 h-4" />
          </button>
          ...
          <button
            type="button"
            onClick={onLike}
            className="absolute top-4 inset-s-4 w-9 h-9 rounded-full bg-black/45 backdrop-blur-sm flex items-center justify-center"
            aria-label={t.like}
          >
            <Heart
              className="w-4 h-4"
          ...
          {!available && (
            <div className="absolute bottom-3 inset-s-4 px-2.5 py-1 rounded-full text-[11px] font-bold bg-gray-600 text-white">
              {t.unavailable}
            </div>
          )}
